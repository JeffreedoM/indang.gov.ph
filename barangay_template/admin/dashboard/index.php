...
$infant = $pdo->query("SELECT COALESCE(COUNT(*), 0) FROM resident WHERE barangay_id = $barangayId AND DATEDIFF(CURDATE(), birthdate) BETWEEN 0 AND 365")->fetchColumn();

// Children count (aged 2-12)
$children = $pdo->query("SELECT COALESCE(COUNT(*), 0) FROM resident WHERE barangay_id = $barangayId AND DATEDIFF(CURDATE(), birthdate) BETWEEN 365 AND 4380")->fetchColumn();

// Teens count (aged 13-17)
$teens = $pdo->query("SELECT COALESCE(COUNT(*), 0) FROM resident WHERE barangay_id = $barangayId AND DATEDIFF(CURDATE(), birthdate) BETWEEN 4745 AND 6570")->fetchColumn();

// Adult count (aged 18-59)
$adult = $pdo->query("SELECT COALESCE(COUNT(*), 0) FROM resident WHERE barangay_id = $barangayId AND DATEDIFF(CURDATE(), birthdate) BETWEEN 6575 AND 21900")->fetchColumn();

// Senior count (aged 60 and above)
$senior = $pdo->query("SELECT COALESCE(COUNT(*), 0) FROM resident WHERE barangay_id = $barangayId AND DATEDIFF(CURDATE(), birthdate) >= 21905")->fetchColumn();
