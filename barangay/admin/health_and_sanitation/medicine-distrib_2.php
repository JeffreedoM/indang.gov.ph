...
$batch = $pdo->query("SELECT * FROM medicine_inventory")->fetchAll();
$merged_query = $pdo->query("SELECT * FROM medicine_distribution JOIN medicine_inventory ON medicine_inventory.ID = medicine_distribution.medicine_id
    JOIN resident ON resident.resident_id = medicine_distribution.resident_id AND resident.barangay_id = '$barangayId'")->fetchAll();
    ...
    <main class="main-content">
        ...
        <div class="wrapper officials">
                    ...
                    <div class="relative w-full max-w-2xl max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <table id="inventory" class="row-border hover">
                                    ...
                                    <tbody>
                                        <?php foreach ($batch as $record) { ?>
                                            <tr id="<?php echo $record['ID'] ?>" style="cursor:pointer" data-modal-hide="motherModal" data-modal-target="motherAlert" data-modal-toggle="motherAlert">
                                                <?php if ($record['medicine_availability'] === $notAvailable) { ?>
                                                    <td style="color: gray;"><?php echo $record['ID'] ?></td>
                                                    <td style="color: gray;"><?php echo $record['medicine_name'] ?></td>
                                                    <td style="color: gray;"><?php echo $record['medicine_availability'] ?></td>
                                                    <td style="color: gray;"><?php echo $record['medicine_quantity'] ?></td>
                                                    <td style="color: gray;"><?php echo $record['medicine_expiration'] ?></td>
                                                    <td style="color: gray;"><?php echo $record['medicine_description'] ?></td>
                                                <?php } else { ?>
                                                    <?php $expiryTimestamp = strtotime($record['medicine_expiration']);
                                                    if ($expiryTimestamp < $currentTimestamp) { ?>
                                                        <td style="color: gray;"><?php echo $record['ID'] ?></td>
                                                        <td style="color: gray;"><?php echo $record['medicine_name'] ?></td>
                                                        <td style="color: gray;"><?php echo "<span style='color:red';>Expired</span>"?></td>
                                                        <td style="color: gray;"><?php echo $record['medicine_quantity'] ?></td>
                                                        <td style="color: gray;"><?php echo $record['medicine_expiration'] ?></td>
                                                        <td style="color: gray;"><?php echo $record['medicine_description'] ?></td>
                                                    <?php }  
                                                        ...
                                                        <td style="color: green;"><?php echo $record['medicine_availability'] ?></td>
                                                        <td><?php echo $record['medicine_quantity'] ?></td>
                                                        <td><?php echo $record['medicine_expiration'] ?></td>
            ...
            <div class="modal" id="modal_vaccine">
                <div class="header">
                    <p class="header-text-vacc"><b>Insert distribution record</b></p>
                    ...
                    <form action="./assets/includes/query.php" method="POST" class="add-officials-form" onsubmit="return validateForm()">
                        ...
                        <!-- medicine --> 
                        <div>  
                            <button class="add-resident__button" style="margin-left:-6.7rem; margin-top: .5rem;" data-modal-target="motherModal" data-modal-toggle="motherModal">
                                <label for="position" class="font-medium text-red-500 dark:text-white">Select medicine<i class="fa-solid fa-caret-down ml-1"></i></label>
                            </button>
                            <input type="text" name="medicine_name" id="medicine_name" placeholder="Select medicine above" readonly aria-label="disabled input 2" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500">
