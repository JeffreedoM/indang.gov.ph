...
if (isset($_POST['submitRecord'])) {
    ...
    if ($conn->connect_error) {
        ...
        $inventory_quantity->bind_param("i", $medicine_id);
        ...
        if ($check_stmt->num_rows > 0) {
            if ($medicine_quantity > $current_quantity) {
                ...
                header('Location: ../../medicine-distribution.php');
            } else {
                ...
                $update_stmt->bind_param("ii", $inventory_quantity, $medicine_id);
                $update_stmt->execute();
                $update_stmt->close();

                $stmt2 = $conn->prepare("INSERT INTO medicine_distribution(medicine_id, distrib_quantity, resident_id, distrib_date) VALUES (?,?,?,?)");
                $stmt2->bind_param("siss", $medicine_id, $medicine_quantity, $resident_id, $date);
                $stmt2->execute();
                $stmt2->close();
            }
        }
        ...
        $inventory_availability->bind_param("i", $medicine_id);
        ...
        if ($status === 0) {
            ...
            $status_update->bind_param("si", $NotAvailable, $medicine_id);
