...
$cleanup = $pdo->query("SELECT * FROM report_cleanup WHERE barangay_id = $barangayId")->fetchAll();
                    ...
                    <tbody>
                        <?php foreach ($cleanup as $row) { ?>
                            <tr>
                                ...
                                <td><?php echo $row['mcu_date'] ?></td>
                                <td>
                                    <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                        <a href="./../../pdf/cleanup_pdf.php?view_id=<?php echo $row['mcu_id']; ?>" target="_blank">VIEW</button>
                                    <button onclick="return confirm('Are you sure?')" type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                                        <a href="delete.php?delete_id=<?php echo $row['mcu_id']; ?>"> DELETE</a></button>
                                </td>
                            </tr>
